...
<link type="text/css" rel="stylesheet" href="/css/camp/camp-detail.css">
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=57f35dff503fc90b1fc42984b5f06cab"></script>
<link rel="stylesheet" href="/css/camp/weather.css" />
 ...
 <div class="container">
      ...
      <div class="row">
         ...
         <div class="div-detail">
            ...
            <!--------------메인이미지-----------  -->
            <c:choose>
               <c:when test="${campsite.thumbnail ne null}">
                  <div class="col-7 main-img-box">
                     <img class="main-img" src="${campsite.thumbnail}">
                  </div>
               </c:when>
               <c:otherwise>
                  <div class="col-7 main-img-box">
                     <img class="main-img" src="/img/gimg.png">
                  </div>
               </c:otherwise>
            </c:choose>
            <!------------------------------  -->
            ...
            <div class="row align-items-end col-5 camp-nm-box">

               <div class="col-12 camp-nm-content">${campsite.facltNm}</div>
               <div class="col-12 reserve-like-box">
               <c:if test="${campsite.resveUrl ne null}">
                  <a href="${campsite.resveUrl}">
                     <button type="button" class="yj-reserve-btn">Reserve</button>
                  </a>
               </c:if>
               <c:if test="${campsite.resveUrl eq null}">
            
                     <button type="button" class="yj-reserve-btn">예약불가</button>
                  
               </c:if>

               <!--========================================start Like=============================================-->
               <!------------------------------- 비회원 ----------------------------------->
                  <c:if test="${dto.member_no eq null}">
                     <c:set var="ls" value="${likesize}" />
                     <div class="nomember-like">❤ ${ls}</div>
                  </c:if>
                     ...
                     <c:if test="${dto.member_no ne null}">
                        ...
                        <c:when test="${!empty like}">
                           <c:forEach var="r" items="${like}">
                              <form action="/dislike"
                                 style="height: 35px; width: 130px; font-size: 14px; height: 40px; 
                                 margin-left: 12rem; text-align: center; border: 2.8px solid; 
                                 border-radius: 0.3rem; border-color: #ff0059;">

                                 <input type="hidden" name="campsiteNo"
                                    value="${r.campsiteNo}"> <input type="hidden"
                                    name="memberNo" value="${r.memberNo}">
                                 <c:set var="ls" value="${likesize}" />
                                 <input class="like-btn" type="submit" value="❤">
                                 <div class="like-count">${ls}</div>
                        ...
                        <c:otherwise>
                           <form action="/like"
                              style="height: 35px; width: 130px; font-size: 14px; 
                              height: 40px; margin-left: 12rem; text-align: center; 
                              border: 2.8px solid; border-radius: 0.3rem; border-color: #dbd9d9;">

                              <input type="hidden" name="campsiteNo"
                                 value="${campsite.campsiteNo}"> <input type="hidden"
                                 name="memberNo" value="${dto.member_no}">
                              <c:set var="ls" value="${likesize}" />
                              <input class="nolike-btn" type="submit" value="❤">
                              <div class="like-count">${ls}</div>
                  ...
                  <td class="camp-detail-content"><c:choose>
                     ...
                     <div class="main-line"></div> <c:choose>
                        ...
                        <c:otherwise>
                           ...
                           <p style="line-height: 32px">${campsite.detailIntro}</p>
                           </div>
                           <br>
                        </c:otherwise>
                     </c:choose></td>
         ...
         <div class="map-weather-box">
            ...
            <section class="weather-box" id="weather-box">
                                 ...
                                 <div class="carousel-inner">
                                     <div class="carousel-item active">
                                         <div class="row">
                                             ...
                                             <div class="col-6 justify-content-right"> <img class="img-fluid" src=""> </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
...
<div class="col-text-box">
         ...
         <!-- <img class="star-img"
          ...
          -->
         <div class="review-part" style="display: flex;">
         <c:set var="rCount" value="${reviewCount}" />
         <c:set var="starAvg" value="${star}" />
         <c:set var="fclAvg" value="${fcl}" />
         <c:set var="envAvg" value="${env}" />
         <c:set var="serAvg" value="${ser}" />
             <div>
             <h4>★ ${starAvg} · 후기 ${rCount}개</h4>
             </div>
            <div class="review-btn">
            	
            		<c:if test="${dto.member_no ne null}">
            		      <button type="button" class="yj-review-btn"
            ...
            <tr>
               <td class="review-table-content">
                  ...
                  <c:set var="dateValue" value="${r.createDate}" /> 
                        <div class="review-date" style="color:#31BAE2 "> ${fn:substring(dateValue,0,10)}</div>
                        <img class="ministar-img"
                           src="data:image/png;base64,">
                        <div>${r.starRating}</div><br>
                        <div><span style="font-weight : 600">시설:</span> ${r.fclRating}</div>
                        <div><span style="font-weight : 600">환경:</span> ${r.envRating}</div>
                        <div><span style="font-weight : 600">서비스:</span> ${r.serviceRating}</div><br>
                        <div>${r.content}</div>
 ...
 <!--  
   ...
   <input type="hidden" value="${campsite.doNm}" id="doNm"/>
   <input type="hidden" value="${campsite.sigungu}" id="sigungu"/>
   <!--==============================================================================-->
</body>
   <style>
       .t-modal{
...
function reviewWrite() {
   location.href="/review?no=" + "${campsite.campsiteNo}" + "&nm=" + "${campsite.facltNm}";
}
